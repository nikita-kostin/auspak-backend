...
def list_stops(current_user: User = Depends(get_current_user)):
    if current_user.entity == UserEntity.manager:
        busesList = supabase.table("buses").select("*").eq("is_active", True).execute().data
        stopsList = supabase.table("stops").select("*").eq("is_active", True).in_("entity", ["parcel_pickup", "parcel_dropoff"]).execute().data
    else:
